...
class MarkdownTableExtractor(AsyncNode):
    ...
    async def prep_async(self, shared):
        print(f"📊 [MarkdownTableExtractor] Preparing to extract tables from message")
        return {
    ...
    async def exec_async(self, prep_res):
        print(f"🔍 [MarkdownTableExtractor] Analyzing text for markdown tables...")
        
        # Check both LLM response and original content
        text_to_analyze = prep_res["llm_response"] or prep_res["content"]
        
        if not text_to_analyze:
            print(f"⚠️ [MarkdownTableExtractor] No text to analyze")
        ...
        if tables:
            ...
            for i, table_text in enumerate(tables):
                ...
                print(f"📋 [MarkdownTableExtractor] Table {i+1}: {len(parsed_table['headers'])} columns, {len(parsed_table['rows'])} rows -> {filename}")
            ...
            print(f"❌ [MarkdownTableExtractor] No markdown tables found")
            return {
                "has_table": False,
                "tables": [],
                "processed_text": text_to_analyze,
    ...
    async def post_async(self, shared, prep_res, exec_res):
        ...
        if exec_res["has_table"]:
            print(f"📊 [MarkdownTableExtractor] Extracted {exec_res['table_count']} table(s)")
            ...
            print(f"📊 [MarkdownTableExtractor] No tables found")
            return "no_tables"