...
class FetchDiscordHistory(AsyncNode):
    ...
    async def prep_async(self, shared):
        print(f"🔍 [FetchDiscordHistory] Starting prep_async")
        print(f"🔍 [FetchDiscordHistory] Input shared data: {shared}")
        print(f"🔍 [FetchDiscordHistory] Preparing to fetch history for channel {shared['channel_id']}, message {shared['message_id']}")
    ...
    async def exec_async(self, prep_res):
        print(f"📥 [FetchDiscordHistory] Starting exec_async")
        ...
        if not channel:
            try:
                print(f"🔍 [FetchDiscordHistory] Channel not in cache, fetching from API...")
                channel = await self.bot.fetch_channel(prep_res["channel_id"])
                print(f"📥 [FetchDiscordHistory] Channel fetched from API: {channel}")
            except discord.NotFound:
                print(f"❌ [FetchDiscordHistory] Channel not found: {prep_res['channel_id']}")
                return []
            except discord.Forbidden:
                print(f"❌ [FetchDiscordHistory] No permission to access channel: {prep_res['channel_id']}")
                return []
        ...
        print(f"📥 [FetchDiscordHistory] Channel found: {channel_name} (ID: {channel.id})")
        
        try:
            print(f"📨 [FetchDiscordHistory] Fetching target message {prep_res['message_id']}")
            target = await channel.fetch_message(prep_res["message_id"])
            print(f"📨 [FetchDiscordHistory] Target message found: {target.id} by {target.author}")
            print(f"📨 [FetchDiscordHistory] Target message content preview: {target.content[:100]}...")
            
            print(f"📜 [FetchDiscordHistory] Fetching {self.history_limit} messages before target")
            ...
            print(f"🔍 [FetchDiscordHistory] Fetched {len(msgs)} messages, checking for [new chat] marker")
            ...
            for i, msg in enumerate(msgs):
                if "[new chat] ---" in msg.content:
                    ...
                    msgs = msgs[:i]
                    print(f"✂️ [FetchDiscordHistory] Found [new chat] marker, cut history to {len(msgs)} messages (everything before and including marker removed)")
                    break
            
            # Reverse to get chronological order (oldest to newest)
            msgs.reverse()
            print(f"✅ [FetchDiscordHistory] Successfully processed {len(msgs)} messages")
            ...
            for i, msg in enumerate(msgs):
                print(f"📜 [FetchDiscordHistory] Message {i+1}: {msg.id} by {msg.author} - {msg.content[:50]}...")
    ...
    async def _extract_table_attachments(self, msgs):
        ...
        print(f"🔄 [FetchDiscordHistory] Extracting table attachments from {len(msgs)} messages")
        table_content_map = {}
        ...
        for msg in msgs:
            ...
            for attachment in msg.attachments:
                if attachment.filename.startswith("daia_replaced_table_") and attachment.filename.endswith(".md"):
                    ...
                    if len(filename_parts) >= 5:
                        ...
                        if message_id_part.isdigit() and count_part.isdigit():
                            ...
                            print(f"📥 [FetchDiscordHistory] Found table attachment: {attachment.filename} with key {attachment_key}")
                                ...
                                async with aiohttp.ClientSession() as session:
                                    async with session.get(attachment.url) as response:
                                        if response.status == 200:
                                            ...
                                            table_content_map[attachment_key] = table_content
                                            print(f"✅ [FetchDiscordHistory] Downloaded and stored table content for key {attachment_key} ({len(table_content)} chars)")
                                        else:
                                            print(f"❌ [FetchDiscordHistory] Failed to download attachment {attachment.filename}: HTTP {response.status}")
                            except Exception as e:
                                print(f"❌ [FetchDiscordHistory] Error downloading attachment {attachment.filename}: {e}")
        
        print(f"✅ [FetchDiscordHistory] Completed extracting table attachments, found {len(table_content_map)} tables.")
        return table_content_map
    
    async def post_async(self, shared, prep_res, exec_res):
        print(f"🔄 [FetchDiscordHistory] Starting post_async")
        
        if exec_res is None:
            print("❌ [FetchDiscordHistory] exec_res is None, setting result to failed")
            return "failed"
            
        print(f"🔄 [FetchDiscordHistory] Exec result contains {len(exec_res.get('messages', []))} messages and {len(exec_res.get('table_map', {}))} table mappings.")
        ...
        print(f"🔄 [FetchDiscordHistory] Updated shared data with message_history and table_content_map")
