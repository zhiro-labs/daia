...
class LLMChat(AsyncNode):
    def __init__(self, genai_client, chat_model, temperature, genai_tools, provider="gemini"):
        super().__init__()
    ...
    async def prep_async(self, shared):
        print(f"🤖 [LLMChat] Preparing chat with {len(shared.get('formatted_history', []))} history messages")
    ...
    async def exec_async(self, prep_res):
        ...
        print(f"🔧 [LLMChat] Using provider: {self.provider}, model: {self.chat_model}, temperature: {self.temperature}")
