...
class TableImageRenderer(AsyncNode):
    ...
    async def prep_async(self, shared):
        ...
        print(f"🖼️ [TableImageRenderer] Preparing to render tables as images")
        print(f"🔍 [TableImageRenderer] Found {len(extracted_tables)} extracted tables")
        print(f"📝 [TableImageRenderer] Response length: {len(llm_response)} characters")
        
        for i, table in enumerate(extracted_tables):
            print(f"📊 [TableImageRenderer] Table {i+1}: valid={table.get('parsed', {}).get('valid', False)}")
    ...
    async def exec_async(self, prep_res):
        print(f"🎨 [TableImageRenderer] Rendering {len(prep_res['extracted_tables'])} table(s)")
        
        if not prep_res["extracted_tables"]:
            print(f"⚠️ [TableImageRenderer] No tables to render")
            return {
                "images": [],
                "response_without_tables": prep_res["llm_response"]
            }
        ...
        for table in prep_res["extracted_tables"]:
            ...
            table_count = table_index + 1 if isinstance(table_index, int) else table_index
            ...
            if table["parsed"]["valid"]:
                try:
                    ...
                    print(f"📋 [TableImageRenderer] Table {table_count}: {len(headers)} headers, {len(rows)} rows")
                    print(f"📋 [TableImageRenderer] Headers: {headers}")
                    
                    # Render table as image
                    print(f"🎨 [TableImageRenderer] Starting image rendering for table {table_count}")
                    ...
                    if table_raw_text and table_raw_text in response_text:
                        ...
                        response_text = response_text.replace(table_raw_text, placeholder)
                        print(f"🔄 [TableImageRenderer] Replaced table {table_count} with placeholder")
                    
                    print(f"✅ [TableImageRenderer] Successfully rendered table {table_count}")
                    
                except Exception as e:
                    print(f"❌ [TableImageRenderer] Failed to render table {table_count}: {e}")
                    print(f"🔍 [TableImageRenderer] Table data: {table.get('parsed', {})}")
            else:
    ...
    def _get_font(self, size: int, bold: bool = False):
        ...
        try:
            ...
            try:
                ...
                print(f"⚠️ [TableImageRenderer] Could not load font from either path: {e}, {e2}, using default")
    ...
    def _wrap_text_with_formatting(self, text: str, max_width: int, fonts) -> List[List[tuple]]:
    ...
    def _calc_col_widths(self, headers, rows, font, padding: int = 36):
        ...
        print(f"📏 [TableImageRenderer] Calculating high-resolution column widths for {len(headers)} columns")
        
        draw = ImageDraw.Draw(Image.new("RGB", (1, 1)))
        col_widths = []
        for i, header in enumerate(headers):
            max_width = max(360, int(draw.textlength(header, font=font)) + padding * 2)  # 3x larger min width
            for row in rows:
                if i < len(row):
                    text = str(row[i]).strip()
                    text_width = int(draw.textlength(text, font=font)) + padding * 2
                    max_width = max(max_width, min(text_width, 1200))  # 3x larger max width
            col_widths.append(max_width)
            print(f"📏 [TableImageRenderer] Column {i+1} ({header}): width={max_width}px")
        
        print(f"📏 [TableImageRenderer] Total table width: {sum(col_widths) + len(col_widths) + 1}px")
        return col_widths

    def _draw_header(self, draw, headers, col_widths, header_height, fonts, colors, padding):
        """Draw the table header row with bold formatting support."""
        x = 0
        for i, (header, width) in enumerate(zip(headers, col_widths)):
            draw.rectangle([x, 0, x + width, header_height],
                        fill=colors["header_bg"], outline=colors["border"])
            
            # Use formatting-aware text wrapping for headers
            header_lines = self._wrap_text_with_formatting(header, width - padding * 2, fonts)
            ...
            for line_segments in header_lines:
                ...
                for segment_type, word in line_segments:
                    font = fonts["header_bold"] if segment_type == 'bold' else fonts["header"]
    ...
    def _draw_rows(self, draw, rows, processed_data, col_widths, start_y, fonts, colors, padding):
        ...
        for row_idx, (processed_row, row_height) in enumerate(processed_data):
            ...
            for i, (wrapped_lines, width) in enumerate(zip(processed_row, col_widths)):
                draw.rectangle([x, y, x + width, y + row_height],
                            fill=row_bg, outline=colors["border"])
    ...
    def _render_table_image(self, table_data: Dict[str, Any]) -> io.BytesIO:
        ...
        print(f"🎨 [TableImageRenderer] Starting table image rendering")
        print(f"📊 [TableImageRenderer] Table dimensions: {len(headers)} columns × {len(rows)} rows")
        
        # Safety check
        if not headers:
            print(f"❌ [TableImageRenderer] No headers found in table data")
        ...
        print(f"🔤 [TableImageRenderer] Loading high-resolution fonts with size {font_size}")
        fonts = {
        ...
        print(f"📐 [TableImageRenderer] High-resolution layout settings: padding={padding}, header_height={header_height}")
        
        col_widths = self._calc_col_widths(headers, rows, fonts["cell"], padding)

        # Process rows
        print(f"📝 [TableImageRenderer] Processing row text wrapping")
        processed_data = []
        for row_idx, row in enumerate(rows):
            processed_row, row_height = [], min_cell_height
            for i, cell in enumerate(row):
                wrapped = self._wrap_text_with_formatting(str(cell).strip(), col_widths[i] - padding * 2, fonts)
        ...
        # Draw table
        print(f"🎨 [TableImageRenderer] Drawing header row")
        self._draw_header(draw, headers, col_widths, header_height, fonts, colors, padding)
        
        print(f"🎨 [TableImageRenderer] Drawing {len(rows)} data rows")
        self._draw_rows(draw, rows, processed_data, col_widths, header_height + 1, fonts, colors, padding)

        # Save high resolution image
        print(f"💾 [TableImageRenderer] Saving high-resolution image to buffer")
        buffer = io.BytesIO()
        img.save(buffer, format="PNG", quality=100, optimize=False, dpi=(300, 300))  # High DPI for crisp output
        buffer.seek(0)
        
        print(f"✅ [TableImageRenderer] High-resolution image rendering complete, buffer size: {len(buffer.getvalue())} bytes")
        return buffer
    ...
    async def post_async(self, shared, prep_res, exec_res):
        ...
        print(f"🔄 [TableImageRenderer] Post-processing results")
        print(f"🖼️ [TableImageRenderer] Images generated: {len(images)}")
        print(f"📝 [TableImageRenderer] Response text length: {len(response_text)} characters")
        
        shared["table_images"] = images
        shared["response_without_tables"] = response_text
        
        for img in images:
            print(f"🖼️ [TableImageRenderer] Image {img['index'] + 1}: {img['filename']}, size: {len(img['buffer'].getvalue())} bytes")
        
        if rendered_count > 0:
            print(f"✅ [TableImageRenderer] Successfully rendered {rendered_count} table image(s)")
            return "images_rendered"
        else:
            print(f"⚠️ [TableImageRenderer] No images rendered")
            return "no_images"