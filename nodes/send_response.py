...
class SendDiscordResponse(AsyncNode):
    ...
    async def prep_async(self, shared):
        ...
        response_text = shared.get("response_without_tables") or shared.get("llm_response", "No response generated")
        table_images = shared.get("table_images", [])
        extracted_tables_files = shared.get("extracted_tables_files", [])
        
        print(f"� [SenndDiscordResponse] Preparing to send response to channel {shared['channel_id']}")
        ...
        print(f"📋 [SendDiscordResponse] Table files to send: {len(extracted_tables_files)}")
        
        return {
            "channel_id": shared["channel_id"],
            "message_id": shared.get("message_id"),  # Add original message ID for replies
    ...
    async def exec_async(self, prep_res):
        ...
        if not channel:
            try:
                print(f"🔍 [SendDiscordResponse] Channel not in cache, fetching from API...")
                channel = await self.bot.fetch_channel(prep_res["channel_id"])
            except (discord.NotFound, discord.Forbidden) as e:
                print(f"❌ [SendDiscordResponse] Cannot access channel {prep_res['channel_id']}: {e}")
                return False
        
        if channel:
            try:
                print(f"📤 [SendDiscordResponse] Sending message to Discord...")
                ...
                if prep_res["table_images"]:
                    ...
                    print(f"🖼️ [SendDiscordResponse] Prepared {len(prep_res['table_images'])} image attachments")
                ...
                if prep_res["extracted_tables_files"]:
                    for table_file in prep_res["extracted_tables_files"]:
                        ...
                        if os.path.exists(table_file):
                            ...
                            print(f"📋 [SendDiscordResponse] Added table file: {filename}")
                        else:
                            print(f"⚠️ [SendDiscordResponse] Table file not found: {table_file}")
                    print(f"📋 [SendDiscordResponse] Prepared {len([f for f in prep_res['extracted_tables_files'] if os.path.exists(f)])} table file attachments")
                
                print(f"📎 [SendDiscordResponse] Total attachments: {len(files)}")
                
                # Split message if it's too long
                message_chunks = split_message(prep_res["response_text"])
                print(f"📝 [SendDiscordResponse] Message split into {len(message_chunks)} chunks")
                
                # Get the original message to reply to, if available
                original_message = None
                if prep_res.get("message_id"):
                    try:
                        original_message = await channel.fetch_message(prep_res["message_id"])
                    except (discord.NotFound, discord.Forbidden) as e:
                        print(f"⚠️ [SendDiscordResponse] Could not fetch original message {prep_res['message_id']}: {e}")
                ...
                if not message_chunks:
                    ...
                    if files:
                        if original_message:
                            ...
                            print(f"✅ [SendDiscordResponse] Sent {len(files)} files as a reply (no text).")
                        else:
                            await channel.send(files=files)
                            print(f"✅ [SendDiscordResponse] Sent {len(files)} files (no text).")
                    ...
                    if original_message:
                        ...
                        print(f"✅ [SendDiscordResponse] Single chunk with {len(files)} files sent as reply")
                    else:
                        await channel.send(content=message_chunks[0], files=files)
                        print(f"✅ [SendDiscordResponse] Single chunk with {len(files)} files sent (fallback)")
                else:
                    ...
                    if original_message:
                        ...
                        print(f"✅ [SendDiscordResponse] First chunk sent as reply")
                    else:
                        await channel.send(message_chunks[0])
                        print(f"✅ [SendDiscordResponse] First chunk sent (fallback)")
                    
                    # Send middle chunks (if any)
                    for i, chunk in enumerate(message_chunks[1:-1], 2):
                        await channel.send(chunk)
                        print(f"✅ [SendDiscordResponse] Chunk {i}/{len(message_chunks)} sent")
                        
                    # Send the last chunk with all the files
                    await channel.send(content=message_chunks[-1], files=files)
                    print(f"✅ [SendDiscordResponse] Last chunk ({len(message_chunks)}/{len(message_chunks)}) with {len(files)} files sent")
                    ...
                    for table_file in prep_res["extracted_tables_files"]:
                        try:
                            if os.path.exists(table_file):
                                ...
                                print(f"🗑️ [SendDiscordResponse] Deleted table file: {table_file}")
                        except Exception as e:
                            print(f"⚠️ [SendDiscordResponse] Failed to delete table file {table_file}: {e}")
                    print(f"🧹 [SendDiscordResponse] Cleanup completed for {len(prep_res['extracted_tables_files'])} table files")
