...
class DiscordTextSplitter:
    ...
    def __init__(self, max_chars: int = 2000, preserve_formatting: bool = True, verbose: bool = False):
    ...
    def _find_safe_split_point(self, text: str, target_pos: int) -> Optional[int]:
        ...
        if crossing_syntax:
            ...
            if syntax_length > self.max_chars:
                if self.verbose:
                    print(f"Warning: {crossing_syntax.name} syntax is {syntax_length} characters, "
    ...
    def _find_all_paired_syntaxes(self, text: str) -> List[SyntaxBoundary]:
        ...
        for boundary in all_boundaries:
            is_overlapped = any(
                (existing.start <= boundary.start < existing.end or 
                 existing.start < boundary.end <= existing.end or
                 (boundary.start <= existing.start and boundary.end >= existing.end))
    ...
    def _find_paired_syntax(self, text: str, opening: str, closing: str, name: str) -> List[SyntaxBoundary]:
        ...
        while pos < len(text):
            ...
            boundaries.append(SyntaxBoundary(open_pos, close_end, name, opening, closing))
            pos = close_end
    ...
    def _find_best_split_before_position(self, text: str, max_pos: int, 
                                       paired_boundaries: List[SyntaxBoundary], 
                                       line_start_formats: List[Tuple[int, str]]) -> int:
        ...
        for i in range(max_pos - 1, -1, -1):
            if text[i] in '.!?':
                split_pos = i + 1
                if self._is_position_safe(split_pos, paired_boundaries, line_start_formats, text):
                    return split_pos
        ...
        for line_start, line_end in self._find_line_boundaries(text):
            if line_end <= max_pos:
                if self._is_position_safe(line_end, paired_boundaries, line_start_formats, text):
    ...
    def _is_position_safe(self, pos: int, paired_boundaries: List[SyntaxBoundary], 
                         line_start_formats: List[Tuple[int, str]], text: str) -> bool:
...
def split_discord_text(text: str, max_chars: int = 2000, preserve_formatting: bool = True, 
    ...
    """
    splitter = DiscordTextSplitter(max_chars=max_chars, preserve_formatting=preserve_formatting, verbose=verbose)
    return splitter.split(text)


def split_message(text: str, max_length: int = 2000, preserve_syntax: bool = True) -> List[str]:
    """
