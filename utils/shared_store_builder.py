    ...
    try:
        ...
        for field in int_fields:
            if field in data and not isinstance(data[field], int):
                print(f"❌ [validate_message_data_types] {field} is not int: {type(data[field])}")
                return False
        
        # Check optional integer fields
        if data.get("guild_id") is not None and not isinstance(data["guild_id"], int):
            print(f"❌ [validate_message_data_types] guild_id is not int: {type(data['guild_id'])}")
            return False
        
        # Check string type fields
        string_fields = ["content", "author_name", "channel_name", "guild_name", "timestamp"]
        for field in string_fields:
            if field in data and not isinstance(data[field], str):
                print(f"❌ [validate_message_data_types] {field} is not str: {type(data[field])}")
                return False
        ...
        for field in list_fields:
            if field in data and not isinstance(data[field], list):
                print(f"❌ [validate_message_data_types] {field} is not list: {type(data[field])}")
                return False
        
        print(f"✅ [validate_message_data_types] All types validated successfully")
        return True
